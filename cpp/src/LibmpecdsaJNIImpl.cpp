...
//);
    char *k = (char*) env->GetStringUTFChars(keygen_result, nullptr);
...
//);
    char *c = (char*) env->GetStringUTFChars(commits, nullptr);
    const jint *cl = env->GetIntArrayElements(commits_length, nullptr);
    char *m = (char*) env->GetStringUTFChars(m_a_ks, nullptr);
...
//);
    char *mg = (char*) env->GetStringUTFChars(m_b_gamma_rec, nullptr);
    const jint *mgl = env->GetIntArrayElements(m_b_gamma_length, nullptr);
    char *mw = (char*) env->GetStringUTFChars(m_b_wi_rec, nullptr);
...
//);
    char *d = (char*) env->GetStringUTFChars(delta_i_rec, nullptr);
...
//);
    char *d = (char*) env->GetStringUTFChars(decommit_rec, nullptr);
...
//);
    char *rr = (char*) env->GetStringUTFChars(R_rec, nullptr);
    const jint *rrl = env->GetIntArrayElements(R_length, nullptr);
    char *rd = (char*) env->GetStringUTFChars(R_dash_rec, nullptr);
    const jint *rdl = env->GetIntArrayElements(R_dash_length, nullptr);
    char *p = (char*) env->GetStringUTFChars(phase5_proof_rec, nullptr);
 ...
 * Signature: (JLjava/lang/String;[ILjava/lang/String;[ILjava/lang/String;[I[ILjava/lang/String;)Ljava/lang/String;
 */
JNIEXPORT jstring JNICALL Java_org_tron_common_tss_Libmpecdsa_00024LibmpecdsaJNI_libmpecdsaSignRound7
    (JNIEnv * env, jobject, jlong ctx, jstring S_rec, jintArray S_length, jstring homo_proof_rec,
     jintArray homo_proof_length, jstring T_i_rec, jintArray T_i_length, jintArray sig_s_i_length,
     jstring message) {
...
//);
    char *s = (char*) env->GetStringUTFChars(S_rec, nullptr);
    const jint *sl = env->GetIntArrayElements(S_length, nullptr);
    char *h = (char*) env->GetStringUTFChars(homo_proof_rec, nullptr);
    const jint *hl = env->GetIntArrayElements(homo_proof_length, nullptr);
    char *t = (char*) env->GetStringUTFChars(T_i_rec, nullptr);
    const jint *tl = env->GetIntArrayElements(T_i_length, nullptr);
    jint *ss = env->GetIntArrayElements(sig_s_i_length, nullptr);
    char *m = (char*) env->GetStringUTFChars(message, nullptr);

    if (s == NULL || sl == NULL || h == NULL || hl == NULL ||
        t == NULL || tl == NULL || ss == NULL || m == NULL) {
        return NULL;
    }

    char *r = libmpecdsa_sign_round7((void *)ctx, s, sl, h, hl, t, tl, ss, m);
    jstring result = env->NewStringUTF(r);
...
//);
    char *l = (char*) env->GetStringUTFChars(local_sig_rec, nullptr);
    const jint *ll = env->GetIntArrayElements(local_sig_length, nullptr);
    char *s = (char*) env->GetStringUTFChars(s_i_rec, nullptr);
